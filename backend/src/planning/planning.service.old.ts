...
import { Cacheable, EvictUserCache, EvictPlanCache } from '../common/cache/cache.decorators';
...
export class PlanningService {
  ...
  async generatePlan(data: PlanGenerationData | (any & { userId: string })): Promise<any> {
    try {
      ...
      // 2. AI ile plan üret
      // Weekly isteklerde 7 gün zorunlu, initial gibi isteklerde planDurationDays taşınabilir
      const isWeekly = String((data as any)?.planType || '').toLowerCase() === 'weekly';
      ...
      let planResult = await this.planGeneration.generatePlan?.({
        ...
        planDurationDays,
        planType: isWeekly ? 'weekly' : (data as any)?.planType,
      ...
      let savedPlan = await this.planPersistence.createPlan({
        userId: (data as any).userId,
        plan: {
          title: safePlan.title || `${((data as any).subjects || []).join(', ')} Çalışma Planı`,
          description: safePlan.description || 'Plan',
          subjects: (data as any).subjects,
          goals: (data as any).goals,
          duration: planDurationDays || (isWeekly ? 7 : 7),
        },
      ...
      // Outbox event: PLAN_CREATED (delta-sync: added plan)
      try {
        const outbox = (this as any).outboxService;
        if (outbox?.enqueue) {
  ...
  async updatePlan(userId: string, planId: string, data: any) {
    let updated = await this.planPersistence.updatePlan(userId, planId, data);
    if (!updated) {
  ...
  async deletePlan(userId: string, planId: string) {
    const result = await this.planPersistence.deletePlan(userId, planId);
    try { await this.cache.delete(`plan:${planId}`); } catch {}
    return result;
  }
