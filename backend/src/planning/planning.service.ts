...
import { SUBJECT_ALIASES } from './planning.constants';
...
- YENİ TALİMAT: Zayıf alanlarda (focusAreas) belirtilen bir konu, havuzdaki bir konuyla birebir aynı olmasa bile, anlamsal olarak en yakın olanı seç. Örneğin, zayıf alan 'Organik Kimya' ise, havuzdan 'Organik Bileşikler' konusunu seçmek önceliğindir.
${topicPoolJson}
...
`;
  ...
  private async buildCurriculumTopicPool(
    ...
    console.log(`\n--- [Boş Raf Operasyonu] ---`);
    console.log(`[1/5] buildCurriculumTopicPool BAŞLADI. Gelen parametreler:`);
    console.log(`  - Sınıf (grade): ${grade}`);
    console.log(`  - Dersler (subjects):`, subjects);

    // Override öncelikli (mantık aynı kalacak)
    const pool: Record<string, string[]> = {};
    if (overrideTopics && Object.keys(overrideTopics).length > 0) {
      console.log('Override konuları tespit edildi. Doğrudan override kullanılacak.');
      Object.entries(overrideTopics).forEach(([subject, topics]) => {
        if (Array.isArray(topics) && topics.length > 0) {
          pool[subject] = topics;
        }
      });
      console.log(`[5/5] Override ile konu havuzu oluşturuldu:`, pool);
    ...
    console.log(`[2/5] Veritabanı sorgusu hazırlanıyor. Aranan sınıf: ${grade}, Aranan dersler:`, normalizedSubjects);
    ...
    console.log(`[3/5] Veritabanı sorgusu tamamlandı. Bulunan toplam konu sayısı: ${topicsFromDb.length}`);
    if (topicsFromDb.length < 5) {
        // Eğer çok az sonuç varsa, ilk birkaçını loglayalım
        console.log('   -> Bulunan ilk birkaç konu:', topicsFromDb.slice(0, 5));
    }
    ...
    subjects.forEach(subject => {
      ...
      console.log(`[4/5] '${subject}' dersi için filtreleme yapılıyor. Bulunan konu sayısı: ${subjectTopics.length}`);

      if (subjectTopics.length > 0) {
        pool[subject] = subjectTopics;
      } else {
        console.error(`  --> DİKKAT: '${subject}' dersi için veritabanında HİÇ konu bulunamadı! Fallback kullanılıyor.`);
        pool[subject] = ['Giriş', 'Temel kavramlar', 'Pekiştirme uygulamaları'];
      }
    });

    console.log(`[5/5] Konu havuzu (pool) oluşturuldu. Sonuç:`, pool);
    console.log(`--- [Operasyon Tamamlandı] ---\n`);
  ...
  private pickTopicFromPool(
    ...
    const normalizeAndTokenize = (s: string): Set<string> => new Set((s || '')
      .toLowerCase()
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
      .trim()
    ...
    const normalizedList = list.map(t => ({ raw: t, tokens: normalizeAndTokenize(t) }));
    const normalizedPrefs = (preferredTopics || [])
      .filter(Boolean)
      .map(p => ({ raw: p, tokens: normalizeAndTokenize(String(p)) }));

    // 1) Odak eşleşmesi (kelime kesişimi) ve recent tekrarı engelle
    for (const pref of normalizedPrefs) {
      const hit = normalizedList.find(nt => {
        const intersection = new Set([...nt.tokens].filter(x => pref.tokens.has(x)));
