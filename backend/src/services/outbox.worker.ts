...
export class OutboxWorker {
  ...
  async processBatch(limit = 100) {
    ...
    for (const ev of events) {
      try {
        await (this.prisma as any).outboxEvent.update({ where: { id: ev.id }, data: { status: 'PROCESSING' } });
        const payload = {
          ...((ev.payload as any) || {}),
        ...
        await (this.prisma as any).outboxEvent.update({ where: { id: ev.id }, data: { status: 'PROCESSED', processedAt: new Date() } });
        ...
        await (this.prisma as any).outboxEvent.update({ where: { id: ev.id }, data: { status: 'PENDING', attemptCount: attempt } });
