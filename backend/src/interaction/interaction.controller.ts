...
export class InteractionController {
  ...
  @Post('chat')
  async handleChat(@Request() req, @Body() body: { message: string; context?: string }) {
    return this.interactionService.handleChat(req.user.id, body.message, body.context);
  }

  @Post('feedback')
  async processFeedback(@Request() req, @Body() body: { type: string; content: string; rating?: number }) {
    return this.interactionService.processFeedback(req.user.id, body.type, body.content, body.rating);
  }

  @Post('support')
  async handleSupport(@Request() req, @Body() body: { issue: string; priority: string }) {
