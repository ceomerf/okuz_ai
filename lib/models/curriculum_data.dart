class CurriculumData {
  ...
  static List<String> getGrades() {
    return curriculum.map((level) => level['sinif_duzeyi'] as String).toList();
  }

  static List<String> getSubjects(String grade) {
    final gradeData = curriculum.firstWhere(
      (level) => level['sinif_duzeyi'] == grade,
      orElse: () => <String, Object>{},
    );
    
    if (gradeData.isEmpty) return [];
    
    final dersler = gradeData['dersler'] as List<dynamic>? ?? [];
    return dersler.map((ders) => ders['ders_adi'] as String).toList();
  }

  static List<String> getTopics(String grade, String subject) {
    final gradeData = curriculum.firstWhere(
      (level) => level['sinif_duzeyi'] == grade,
      orElse: () => <String, Object>{},
    );
    ...
    final dersler = gradeData['dersler'] as List<dynamic>? ?? [];
    final ders = dersler.firstWhere(
      (ders) => ders['ders_adi'] == subject,
      orElse: () => <String, Object>{},
    );
    
    if (ders.isEmpty) return [];
    
    List<String> allTopics = [];
    
    // Temalar varsa
    if (ders.containsKey('temalar')) {
      final temalar = ders['temalar'] as List<dynamic>? ?? [];
      for (var tema in temalar) {
        final konular = tema['konular'] as List<dynamic>? ?? [];
        allTopics.addAll(konular.map((k) => k as String));
      }
    }
    
    // Üniteler varsa
    if (ders.containsKey('uniteler')) {
      final uniteler = ders['uniteler'] as List<dynamic>? ?? [];
      for (var unite in uniteler) {
        final konular = unite['konular'] as List<dynamic>? ?? [];
        allTopics.addAll(konular.map((k) => k as String));
      }
    }
  ...
  static String getSubjectNote(String grade, String subject) {
    final gradeData = curriculum.firstWhere(
      (level) => level['sinif_duzeyi'] == grade,
      orElse: () => <String, Object>{},
    );
    ...
    return ders['not'] as String? ?? '';
  }

  static Map<String, String> getSubjectIcons() {
    return {
  ...
  static const Map<String, String> difficultyLevels = {
    'Başlangıç': 'Temel kavramları öğrenme',
    'Orta': 'Kavramları uygulama ve problem çözme',
    'İleri': 'Karmaşık problemler ve analiz',
  };
} 