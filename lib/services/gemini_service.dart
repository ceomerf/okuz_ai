...
// Anahtar --dart-define üzerinden gelmelidir: e.g., --dart-define=GEMINI_API_KEY=...
...
''';
  ...
  Future<String> getStudyPlanFromGemini(String prompt) async {
    ...
    if (apiKey.isEmpty) {
      throw Exception('GEMINI_API_KEY tanımlı değil. Derleme sırasında --dart-define ile sağlayın.');
    }
  ...
  Future<Map<String, dynamic>> generateSummary({
    ...
    try {
      final result = await _apiClient.generateSummary(
        sourceType: 'text',
      ...
      // Backend'den hata mesajı gelirse kontrol et
      if (result['summary'] != null &&
          (result['summary'].toString().contains('hata oluştu') ||
              result['summary'].toString().contains('AI yanıtı işlenirken') ||
              result['summary'].toString().contains('AI özet oluşturamadı'))) {
        throw Exception('AI Hatası: ${result['summary']}');
      }
