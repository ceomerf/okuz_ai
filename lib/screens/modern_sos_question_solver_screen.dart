...
class ModernSOSQuestionSolverScreen extends ConsumerStatefulWidget {
  const ModernSOSQuestionSolverScreen({super.key});

  @override
  _ModernSOSQuestionSolverScreenState createState() =>
      _ModernSOSQuestionSolverScreenState();
}

class _ModernSOSQuestionSolverScreenState extends ConsumerState<ModernSOSQuestionSolverScreen>
    ...
    try {
      ...
      setState(() {
        ...
        if (learningPath != null) {
          ...
          if (learningPath['steps'] is List) {
            // Eğer steps zaten bir liste ise, direkt kullan
            _solutionSteps = List<Map<String, dynamic>>.from(learningPath['steps']);
          } else {
            // Eğer steps bir string ise, JSON parse et
            String stepsJson = learningPath['steps'].toString();
            
            // JSON string'i temizle
            if (stepsJson.contains('```json')) {
              stepsJson = stepsJson.replaceAll('```json', '').replaceAll('```', '').trim();
            }
            
            try {
              // JSON'u parse et
              final List<dynamic> parsedSteps = jsonDecode(stepsJson);
              _solutionSteps = parsedSteps.map((step) => Map<String, dynamic>.from(step)).toList();
            } catch (e) {
              print('JSON parse hatası: $e');
              // Fallback: Tek adım olarak kullan
              _solutionSteps = [
                {
                  'step': 1,
                  'explanation': learningPath['steps'].toString(),
                  'formula': null
                }
              ];
            }
          ...
          // Çözüm metnini oluştur
          _solution = _solutionSteps.map((step) {
            String stepText = '${step['step']}. ${step['explanation']}';
            if (step['formula'] != null && step['formula'].toString().isNotEmpty) {
              stepText += '\n\nFormül: ${step['formula']}';
            }
            return stepText;
          }).join('\n\n');
