...
import 'package:okuz_ai/services/providers.dart' show apiClientProvider;
...
class _FocusModeScreenState extends ConsumerState<FocusModeScreen>
  ...
  void initState() {
    ...
    // Her 1 dakikada bir ilerlemeyi backend'e bildir
    _progressTimer = Timer.periodic(const Duration(minutes: 1), (timer) async {
      if (!mounted) return;
      if (_isRunning && !_isCompleted && (widget.taskId != null)) {
        try {
          final planService = ref.read(planServiceProvider);
          await planService.updateTaskProgress(widget.taskId!, 1);
        } catch (_) {
          // Sessiz geç
        }
      }
  ...
  void _trackStudySession() async {
    ...
    try {
      final studyService = StudyTrackingService(ref.read(apiClientProvider));
      // TODO: StudyDataProvider legacy; Riverpod'a taşınmalı
