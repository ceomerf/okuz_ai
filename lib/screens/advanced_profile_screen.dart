...
class AdvancedProfileScreen extends ConsumerWidget {
  ...
  Future<bool> _checkUserHasPlan(BuildContext context, WidgetRef ref) async {
    final user = MockAuthService.instance.currentUser;
    if (user == null) return false;

    try {
      final apiClient = ref.read(apiClientProvider);
      final planService = PlanService.withClient(apiClient);
      final planDocument = await planService.getUserPlan();
      return planDocument.isNotEmpty;
    } catch (e) {
  ...
  Future<void> _navigateToUserPlan(BuildContext context, WidgetRef ref) async {
    // Plan kontrolü yap
    final hasPlan = await _checkUserHasPlan(context, ref);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          TextButton(
            onPressed: () => _navigateToUserPlan(context, ref),
